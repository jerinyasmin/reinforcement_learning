...
# sys.path.append('../')
# sys.path.insert(0, 'C:/UNI/projects/rlassist/util/')
# conf_path = os.getcwd()
# print("conf path:", conf_path)
# sys.path.append(conf_path)
# sys.path.append(conf_path + '\\util')
...
class Book_Keeper():
    ...
    def show_eval_summary(self, global_episodes, which='eval'):
        ...
        def show_(tag, err_cnts, episode_lengths, edits, spurious_edits, moves, spurious_moves, fix_cnts, rewards, eval_fixed_progs, eval_par_fixed_progs):
            ...
            #print('Errs_last:%-2d |' % np.sum(err_cnts[last:]), 'Fixes_last:%-2d' % np.sum(fix_cnts[last:]))
        ...
        with sess.as_default(), sess.graph.as_default():
            with coord.stop_on_exception():
                while not coord.should_stop() and start < end:
                    ...
                    step = 0
                    output = ''
                    #print("org_err_cnt_eval: ", org_err_cnt)
            ...
            with coord.stop_on_exception():
                while not coord.should_stop() and Worker.global_episodes <= Worker.max_global_episodes:
                    ...
                    if args.verbose:
                        ...
                        sleep(args.verbose_sleep_time)
                    print("org_err_cnt_train: ", org_err_cnt)
                    this_err_cnts_min = this_err_cnts = org_err_cnt
                    ...
                    while episode_step_count < max_episode_length:
                        ...
                        # Guided exploration
                        # print("suggested_actions: ", len(suggested_actions))

                        if suggested_actions is not None:
                            sg_action = list(suggested_actions)
                            print("suggested_actions: ", len(suggested_actions))

                            if len(sg_action) > 0:
                                #print("suggested_actions: if")
                                action = sg_action.pop(0)
                        else:
                            action = self.rng.choice(action_dist[0], p=action_dist[0])
                            action = np.argmax(action_dist == action)
                            # print("suggested_actions: else")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-data_directory', "--data_directory", help='Data directory',
                        default="C:\\UNI\\projects\\rlassist\\data\\network_inputs\\RLAssist-seed-1189\\bin_0\\")
    parser.add_argument('-checkpoints_directory', "--checkpoints_directory", help='Checkpoints directory',
                        default="C:\\UNI\\projects\\rlassist\\data\\checkpoints\\RLAssist-seed-1189\\bin_0\\")
    ...
    parser.add_argument('-e', "--max_epLength", type=int,
                        help="The max allowed length of each episode.", default=10) #100
    ...
    parser.add_argument("-ep", "--epochs", type=int,
                        help="run training for this many epochs", default=1)
    ...
    print("GE-ratio:")
    print(args.GE_ratio)
    print("use_compiler", args.use_compiler)
    ...
    with tf.device("/cpu:0"):
        ...
        print("params_to_save:", params_to_save)
