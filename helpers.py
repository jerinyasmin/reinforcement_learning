...
def remove_non_ascii(text):
    ...
    #return ''.join([i if isinstance(i, str) and len(i) == 1 and ord(i) < 128 else '' for i in text])
...
def compilation_errors(string, which='gcc'):
    ...
    # filename = 'D:/file/tempfile_%d_%d.c' % (name1, name2)
    # #print("filename: ", filename)
    # outfile = 'D:/file/temp.out'

    filename = tmp + '/tmp/tempfile_%d_%d.c' % (name1, name2)
    outfile = tmp + '/tmp/temp.out'
...
class logger():
    ...
    def __init__(self, log_file, move_to_logs_dir=True):
        ...
        if move_to_logs_dir and not self.log_file.startswith('logs/'):
            self.log_file = os.path.join('C:\\UNI\\projects\\rlassist\\logs', self.log_file)
        self.open = False
        self.handle = None
        self._open()
...
def get_rev_dict(dict_):
    ...
    rev_dict_ = {}
    for key, value in dict_.items():     #python 3
        rev_dict_[value] = key
