...
def load_dictionaries(destination, name='all_dicts.npy'):
    tl_dict, rev_tl_dict = np.load(os.path.join(destination, name), allow_pickle=True)
...
class Env_engine:
    ...
    def gui_set_cursor(self, prog_vector, cursor=None):
        ...
        if not cursor:
            if self.top_down_movement:
                ...
                cursor = self.rng.choice(range(len(prog_vector)))
        position = cursor + 1
    ...
    def is_exact_match(self, vector_program1, vector_program2):
        ...
        return self.cmp(vector_program1, vector_program2) == 0
    ...
    def show(self, program_id, program_vector, name_dict, name_seq, original_program_vector, use_compiler):
        ...
        print("error_list:", len(list(err_list)))

        output += ('**NO ERRORS**' if len(list(err_list)) == 0 else 'ERRORS:\n' + '\n'.join(map(str, err_list))) + str(list(err_list)) + '\n'
        return output
    ...
    def get_compiler_errors(self, program_id, program, name_dict, name_seq):
        ...
        #print("program_source: ", program_source)
...
class Environment(Env_engine):
    ...
    def new_indexed_episode(self, index, use_compiler, which='train', code_id=None):
        ...
        #print("code_id_eval1: ", code_id)
        #print("which_eval: ", which)
        #print("index_eval: ", index)
        if code_id is None:
            assert index < size, 'index:%d >= %s-data-size:%d' % (
                index, which, size)
            part = self.code_ids[which]
            #print("part_eval: ", part)
            if not isinstance(part, list):
                part = list(part)
            code_id = part[index]
            #print("code_id_eval2: ", code_id)
            # code_id = list(self.code_ids[which][index])
            # code_id = self.code_ids[which][index]

        name_dict, name_seq = self.get_identifiers(code_id)
        #print("name_dict_eval: ", name_dict)
        #print("name_seq_eval: ", name_seq)
        program, original_program = copy.deepcopy(data[code_id])
        #print("program_eval: ", program)
        #print("original_program_eval: ", original_program)
        ...
        #print("error_count_eval: ", error_count)
    ...
    def new_random_episode(self, use_compiler, which, GE_selection_probability):
        ...
        #print("error_count_train: ", error_count)
...
class load_data:
    def _deserialize(self, data_folder):
        train_ex = np.load(os.path.join(
            data_folder, 'examples-train.npy'), allow_pickle=True,).item()
        valid_ex = np.load(os.path.join(
            data_folder, 'examples-validation.npy'), allow_pickle=True,).item()
        test_ex = np.load(os.path.join(
            data_folder, 'examples-test.npy'), allow_pickle=True,).item()
        ...
        if load_real_test_data:
            try:
                self.real_test_data = np.load(os.path.join(
                    data_folder, 'test_real_raw.npy'), allow_pickle=True).item()
            except:
                self.real_test_data = np.load(os.path.join(
                    data_folder, 'test_raw.npy'), allow_pickle=True).item()
        if load_seeded_test_data:
            try:
                self.seeded_test_data = np.load(os.path.join(
                    data_folder, 'test_real_seeded.npy'), allow_pickle=True).item()
            except:
                self.seeded_test_data = np.load(os.path.join(
                    data_folder, 'test_seeded.npy'), allow_pickle=True).item()
        try:
            self.name_dict_store = np.load(os.path.join(
                data_folder, 'name_dict_store.npy'), allow_pickle=True).item()
        except:
